...
pub async fn list_ai_models(
    ...
    let catalog_provider_id = req.catalog_provider_id.filter(|id| !id.trim().is_empty());
    ...
    let resp = match req_builder.send().await {
        Ok(r) => r,
        Err(e) => {
            return catalog_fallback(format!("模型列表请求失败: {}", format_reqwest_error(&e)))
                .await;
        }
    };
...
mod tests {
    ...
    fn parse_models_response_openai_and_google_shapes() {
        ...
        let google =
            r#"{"models":[{"name":"models/gemini-2.5-flash"},{"name":"models/gemini-2.0-flash"}]}"#;
        assert_eq!(
