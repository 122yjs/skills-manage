...
pub async fn get_all_skill_installations(pool: &DbPool) -> Result<Vec<SkillInstallation>, String> {
    sqlx::query_as::<_, SkillInstallation>("SELECT * FROM skill_installations ORDER BY agent_id, skill_id")
        .fetch_all(pool)
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn get_all_paused_installations(pool: &DbPool) -> Result<Vec<PausedInstallation>, String> {
    sqlx::query_as::<_, PausedInstallation>("SELECT * FROM paused_installations ORDER BY agent_id, skill_id")
        .fetch_all(pool)
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn get_all_agent_skill_observations(
    ...
    sqlx::query_as::<_, AgentSkillObservation>("SELECT * FROM agent_skill_observations ORDER BY agent_id, dir_path")
        .fetch_all(pool)
        .await
        .map_err(|e| e.to_string())
}
