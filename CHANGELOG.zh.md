...
- 将无法访问的私有复用 CI 替换为仓库内的前端构建/测试/Lint 与 Rust 测试/Clippy 检查，并保留只读 Graphify 产物和 PR 重叠检查。
- 移除已损坏的自动 PR 创建工作流；改为手动创建 PR，保留 PR 创建及更新时的自动检查。
- GitHub 导入前识别非中央技能 ID 冲突；冲突项默认跳过，可显式修改本地安装 ID，不修改源 SKILL.md 或已有平台安装。
- 写入前拒绝已占用 ID 和非托管目标路径；当前技能导入失败时恢复原目标，并分别报告已完成、失败和尚未尝试的项目。
- 修复 React 18 下编辑安装 ID 时的输入框聚焦与文本选中。

### 改进

- 持久保存 GitHub 来源，使其在重新扫描后保留；技能卡片与详情展示仓库、源文件链接及本地安装 ID。
- 复用仓库集合来分组导入的技能，集合与来源独立，并区分集合创建/刷新失败和导入失败。
